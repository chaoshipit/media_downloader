...
class DouyinWebCrawler:
    ...
    # 获取用户所有收藏夹列表
    async def fetch_user_collects(self, cursor: int = 0, count: int = 20):
        kwargs = await self.get_douyin_headers()
        base_crawler = BaseCrawler(
            proxies=kwargs["proxies"], crawler_headers=kwargs["headers"]
        )
        async with base_crawler as crawler:
            params = UserCollects(cursor=cursor, count=count)
            params_dict = params.dict()
            params_dict["msToken"] = ""
            a_bogus = BogusManager.ab_model_2_endpoint(
                params_dict, kwargs["headers"]["User-Agent"]
            )
            endpoint = f"{DouyinAPIEndpoints.USER_COLLECTS}?{urlencode(params_dict)}&a_bogus={a_bogus}"
            response = await crawler.fetch_get_json(endpoint)
        return response

    # 获取指定收藏夹内的视频列表
    async def fetch_user_collects_videos(
        self, collects_id: str, cursor: int = 0, count: int = 20
    ):
        kwargs = await self.get_douyin_headers()
        base_crawler = BaseCrawler(
            proxies=kwargs["proxies"], crawler_headers=kwargs["headers"]
        ...
        async with base_crawler as crawler:
            params = UserCollectsVideo(
                collects_id=collects_id, cursor=cursor, count=count
            )
            params_dict = params.dict()
            params_dict["msToken"] = ""
            a_bogus = BogusManager.ab_model_2_endpoint(
            ...
            endpoint = f"{DouyinAPIEndpoints.USER_COLLECTS_VIDEO}?{urlencode(params_dict)}&a_bogus={a_bogus}"
            response = await crawler.fetch_get_json(endpoint)
        return response
    ...
    async def update_cookie(self, cookie: str):
        ...
        service = "douyin"
        print(
            "DouyinWebCrawler before update",
