...
from app.api.models.APIResponseModel import (  # 导入响应模型
    ErrorResponseModel,
    ResponseModel,
)
...
@router.get(
    "/fetch_user_collects",
    response_model=ResponseModel,
    summary="获取用户所有收藏夹列表/Get user collects list",
)
async def fetch_user_collects(
    ...
    # [English]
    ### Purpose:
    - Get user collects list (folders)
    ### Parameters:
    ...
    """
    try:
        data = await DouyinWebCrawler.fetch_user_collects(cursor, count)
        return ResponseModel(code=200, router=request.url.path, data=data)
    except Exception:
        status_code = 400
        detail = ErrorResponseModel(
...
@router.get(
    ...
    response_model=ResponseModel,
    summary="获取指定收藏夹内的视频列表/Get videos in specified collects folder",
)
async def fetch_user_collects_videos(
    request: Request,
    collects_id: str = Query(
        example="7123456789012345678", description="收藏夹ID/Collects folder ID"
    ),
):
    """
    # [中文]
    ...
    - 获取指定收藏夹内的视频列表
    ### 参数:
    - collects_id: 收藏夹ID（可从fetch_user_collects接口获取）
    ### 返回:
    - 收藏夹内的视频列表

    # [English]
    ### Purpose:
    - Get videos in specified collects folder
    ### Parameters:
    - collects_id: Collects folder ID (can be obtained from fetch_user_collects API)
    ### Return:
    - Videos in the collects folder

    # [示例/Example]
    collects_id = "7123456789012345678"
    """
    try:
        raw_data = await DouyinWebCrawler.fetch_user_collects_videos(
            collects_id, cursor=0, count=1000
        )
        ...
        for aweme in aweme_list:
            ...
            web_url = f"https://www.douyin.com/video/{aweme_id}" if aweme_id else ""
        ...
        return JSONResponse({"code": 200, "msg": "success", "video_list": video_list})
    except Exception as e:
        return JSONResponse({"code": 400, "msg": str(e), "video_list": []})
...
async def generate_a_bogus(
    ...
    try:
        ...
        return ResponseModel(code=200, router=request.url.path, data=a_bogus)
    except Exception:
        status_code = 400
        detail = ErrorResponseModel(
            code=status_code,
